...
class SaleOrderBarcode(models.TransientModel):
    ...
    def _prepare_domain(self):
        self.ensure_one()
        domain = [("ean13", "=", self.product_barcode), ("sale_ok", "=", True)]
        return domain
    ...
    def _convert_uom(self, lines, product_qty):
        ...
        return obj_product._compute_qty(
            from_uom_id=from_uom, qty=product_qty, to_uom_id=to_uom
        )
            ...
            if len(products) == 1:
                ...
                return {
                    "warning": {
                        ...
                        % "\n".join([product.name_get()[0][1] for product in products]),
                    }
                }
            else:
                return {
    ...
    def create_sale_order_line(self):
        ...
        if len(lines) == 1:
            ...
            if check_uom is True:
                ...
                product_qty = self._convert_uom(lines, self.product_qty)
            if self.type == "insert":
                lines.write({"product_uom_qty": lines.product_uom_qty + product_qty})
            elif self.type == "update":
                lines.write({"product_uom_qty": product_qty})
        elif len(lines) > 1:
