...
class SaleOrderBlanketWizard(models.TransientModel):
    ...
    def _check_valid_blanket_order_line(self, bo_lines):
        ...
        for line in bo_lines:
            ...
            if float_is_zero(line.remaining_uom_qty, precision_digits=precision):
    ...
    def _default_lines(self):
        ...
        lines = [(0, 0, self._prepare_order_line_data(line)) for line in bo_lines]
    ...
    blanket_order_id = fields.Many2one(
        string="# Blanket Order", comodel_name="sale.order.blanket", readonly=True
    )
    sale_order_id = fields.Many2one(
        comodel_name="sale.order", string="Sale Order", domain=[("state", "=", "draft")]
    )
    ...
    def create_sale_order(self):
        ...
        for order_line in order_lines:
            order_vals = self._prepare_sale_order_data(order_line, order_lines)
    ...
    @api.multi
    def _prepare_sale_order_data(self, order_line, order_lines):
        self.ensure_one()
...
class SaleOrderBlanketWizardLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one(comodel_name="sale.order.blanket.wizard")
    blanket_line_id = fields.Many2one(comodel_name="sale.order.blanket.line")
    blanket_order_id = fields.Many2one(
    ...
    currency_id = fields.Many2one(
        comodel_name="res.currency", related="blanket_line_id.currency_id"
    )
    ...
    taxes_id = fields.Many2many(
        comodel_name="account.tax", related="blanket_line_id.taxes_id", readonly=True
    )
