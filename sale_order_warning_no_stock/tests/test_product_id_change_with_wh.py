...
class TestProductIdChangeWithWh(TransactionCase):
    ...
    def test_product_id_change_with_wh(self):
        ...
        onchange_res = self.registry(
            ...
            self.cr, self.uid, [], self.order.pricelist_id.id,
            self.order_line.product_id.id,
            qty=self.order_line.product_uom_qty,
            uom=self.order_line.product_uom.id, qty_uos=0, uos=False,
            name='', partner_id=self.order.partner_id.id,
            lang=False, update_tax=True, date_order=self.order.date_order,
