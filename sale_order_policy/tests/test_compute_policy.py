...
class TestComputeOrderPolicy(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.so_type_normal = self.env.ref("sale_order_type.normal_sale_type")
        self.group_employee_id = self.ref("base.group_user")
        self.grp_so_manager = self.env.ref("base.group_sale_manager")
    ...
    def _create_user_1(self):
        val = {
            ...
            "notify_email": "none",
            "groups_id": [(6, 0, [self.group_employee_id, self.grp_so_manager.id])],
        }
    ...
    def test_compute_case_admin(self):
        # Create Sale Order
        sale_order = self._create_sale_order(False)
    ...
    def test_compute_case_no_type(self):
        ...
        self.assertEqual(True, sale_order.sudo(self.user_1.id).invoice_recreate_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).invoice_corrected_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).quotation_send_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).confirm_order_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).view_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).create_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).copy_quotation_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).cancel_quot_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).cancel_order_ok)

    def test_compute_case_1(self):
        # Create Sale Order
        sale_order = self._create_sale_order(self.so_type_normal)
        ...
        self.so_type_normal.invoice_recreate_group_ids = [
            (6, 0, [self.grp_invoice_recreate.id])
        ]

        self.so_type_normal.invoice_corrected_group_ids = [
            (6, 0, [self.grp_invoice_corrected.id])
        ]

        self.so_type_normal.quotation_send_group_ids = [
            (6, 0, [self.grp_quotation_send.id])
        ]
        ...
        self.assertEqual(False, sale_order.sudo(self.user_1.id).invoice_recreate_ok)
        self.assertEqual(False, sale_order.sudo(self.user_1.id).invoice_corrected_ok)
        self.assertEqual(False, sale_order.sudo(self.user_1.id).quotation_send_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).confirm_order_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).view_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).create_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_1.id).copy_quotation_ok)
        ...
        #   1. User 1 have group
        # Add Group Recreate Invoice
        self.user_1.groups_id = [(4, self.grp_invoice_recreate.id)]
        # Add Group Ignore Exception
        self.user_1.groups_id = [(4, self.grp_invoice_corrected.id)]
        # Add Group Send by Email
        self.user_1.groups_id = [(4, self.grp_quotation_send.id)]
    ...
    def test_compute_case_2(self):
        ...
        self.so_type_normal.confirm_order_group_ids = [
            (6, 0, [self.grp_confirm_order.id])
        ]

        self.so_type_normal.view_invoice_group_ids = [
            (6, 0, [self.grp_view_invoice.id])
        ]

        self.so_type_normal.create_invoice_group_ids = [
            (6, 0, [self.grp_create_invoice.id])
        ]
        ...
        self.assertEqual(True, sale_order.sudo(self.user_2.id).invoice_recreate_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).invoice_corrected_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).quotation_send_ok)
        self.assertEqual(False, sale_order.sudo(self.user_2.id).confirm_order_ok)
        self.assertEqual(False, sale_order.sudo(self.user_2.id).view_invoice_ok)
        self.assertEqual(False, sale_order.sudo(self.user_2.id).create_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).copy_quotation_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).cancel_quot_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).cancel_order_ok)

        # Condition :
        #   1. User 2 have group
        # Add Group Confirm Sale
        self.user_2.groups_id = [(4, self.grp_confirm_order.id)]
        # Add Group View Invoice
        self.user_2.groups_id = [(4, self.grp_view_invoice.id)]
        # Add Group Create Invoice
        self.user_2.groups_id = [(4, self.grp_create_invoice.id)]
        ...
        self.assertEqual(True, sale_order.sudo(self.user_2.id).invoice_corrected_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).quotation_send_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).confirm_order_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).view_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).create_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).copy_quotation_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).cancel_quot_ok)
        self.assertEqual(True, sale_order.sudo(self.user_2.id).cancel_order_ok)
    ...
    def test_compute_case_3(self):
        ...
        self.so_type_normal.copy_quotation_group_ids = [
            (6, 0, [self.grp_copy_quotation.id])
        ]

        self.so_type_normal.cancel_quot_group_ids = [(6, 0, [self.grp_cancel_quot.id])]

        self.so_type_normal.cancel_order_group_ids = [
            (6, 0, [self.grp_cancel_order.id])
        ]
        ...
        self.assertEqual(True, sale_order.sudo(self.user_3.id).invoice_recreate_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).invoice_corrected_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).quotation_send_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).confirm_order_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).view_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).create_invoice_ok)
        self.assertEqual(False, sale_order.sudo(self.user_3.id).copy_quotation_ok)
        self.assertEqual(False, sale_order.sudo(self.user_3.id).cancel_quot_ok)
        self.assertEqual(False, sale_order.sudo(self.user_3.id).cancel_order_ok)

        # Condition :
        #   1. User 3 have group
        # Add Group New Copy of Quotation
        self.user_3.groups_id = [(4, self.grp_copy_quotation.id)]
        # Add Group Cancel Quotation
        self.user_3.groups_id = [(4, self.grp_cancel_quot.id)]
        # Add Group Cancel Order
        self.user_3.groups_id = [(4, self.grp_cancel_order.id)]
        ...
        self.assertEqual(True, sale_order.sudo(self.user_3.id).view_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).create_invoice_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).copy_quotation_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).cancel_quot_ok)
        self.assertEqual(True, sale_order.sudo(self.user_3.id).cancel_order_ok)
